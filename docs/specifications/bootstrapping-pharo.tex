...
\textbf{Tools} & SystemNavigation, compiler/refactoring tools, Iceberg,
  agent self-modification primitives \\
\bottomrule
\end{tabularx}
\end{center}
...
result.  The Messaging SDK shipped here is the bootstrap artifact
that proves the external-agent path works; Phase~4 capabilities
continue in \texttt{claude-agent-sdk-pharo}, where the first
products built by the in-image agent will land.
